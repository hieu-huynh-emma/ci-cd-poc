if (!customElements.get("affirm-widget")) {
  class AffirmWidget extends CustomElement {
    ...
    onCartTotalChange() {
      const wrapperEl = document.querySelector("#cart-total");
      const cartTotalEl = wrapperEl.querySelector(".af_new_price") || wrapperEl;
      const rawTotal = cartTotalEl.textContent.replace(/[\$,]/g, "");
      const cartTotal = parseFloat(rawTotal).toFixed(2);
