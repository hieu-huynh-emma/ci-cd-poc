...
if (!customElements.get("cart-drawer-items")) {
    class LegacyCartItems extends CustomElement {
        get refs() {
            return {
        ...
        async requestCart({url: requestUrl = routes.cart_change_url, method = 'POST', payload = {}} = {}) {
            const config = {
                method, headers: {
            ...
            const url = new URL(requestUrl, window.location)

            const renderSections = this.getSectionsToRender().map((section) => section.section)
            ...
            return fetch(url.toString(), config).then(res => res.json())
        }
        ...
        async updateCart(payload = {}, url = routes.cart_change_url) {
            ...
            try {
                ...
                window.Cart.state.totalQuantity = state.item_count
        ...
        async updateQuantity($lineItem, payload) {
            ...
            const {cartRecommendation, cartSummary, cartFooter} = this.refs
    ...
    customElements.define("cart-drawer-items", class CartDrawerItems extends LegacyCartItems {
...
if (!customElements.get("cart-item")) {
    customElements.define("cart-item", class CartItem extends CustomElement {
        props = {
            productId: "",
            variantId: "",
            addonService: null,
