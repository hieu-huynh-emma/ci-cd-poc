...
const expressCheckoutMutationObserver = new MutationObserver(function (mutations) {
  expressCheckoutMutationObserver.disconnect();
  mutations.forEach(function (mutation) {
    $(".Xrk_DudB6JJ3t2Kh2_cU").text("Pay with");

    setTimeout(() => {
      $(".paypalLight").parent().css('position', 'relative')
      $(".paypalLight").before(`
        <div class='paypal-express-checkout'>
          <span>Pay with</span>
          
          <div class="ElVMgDjOgshGFMIvg3se">
    ...
    $(".paypal-button-label-container").prepend(`<span class="Xrk_DudB6JJ3t2Kh2_cU">Pay with</span>`);
  });
});
...
(function ($) {
  $(document).on("page:load page:change", function () {
    ...
    if (Shopify.Checkout.step === "contact_information") {
      ...
      if(screenWidth < 769) renderExpressCheckoutButtonTexts();
    }
  });
})(Checkout.$);


function renderExpressCheckoutButtonTexts() {
  expressCheckoutMutationObserver.observe(
    document.querySelector(".dynamic-checkout__buttons"),
    {
      childList: true,
      subtree: true,
    },
