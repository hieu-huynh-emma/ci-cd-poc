...
class VariantSelects extends HTMLElement {
    ...
    updateCrosssell() {
        ...
        const $protectorCrossell = $(`addon-service[data-product-id="7907000287370"]`);

        if (!!$protectorCrossell.length) {
            const protectorId = $protectorCrossell.data("productId");

            const protectorData = JSON.parse(document.querySelector(`script#ProductJson-${protectorId}[type="application/json"]`).textContent);

            const hasCompatibleSize = protectorData.variants.some((v) => {
                const variantSize = v.title.split("|")[0].trim();
                return variantSize === size;
            });

            if (!hasCompatibleSize) {
                $protectorCrossell.hide();
                const isChecked = $protectorCrossell.find(".addon-checkbox__input").is(":checked");
                if (isChecked) {
                    $protectorCrossell.trigger("click");
                }
            } else {
                $protectorCrossell.show();
            }
        }
    }
    ...
    connectedCallback() {
      ...
      `);
        ...
        $(document).ready(async () => {
            starRatingObserver.observe(this.querySelector(".yotpo.bottomLine"), {
                childList: true,
                subtree: true,
            });
        });
